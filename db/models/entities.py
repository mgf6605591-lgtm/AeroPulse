...
class Airport(Base):
    ...
    # Предприятие, в состав которого входит аэропорт. ФКП «Аэропорты Севера»
    # сдаёт один бланк 15-ГА на все свои аэропорты: сводный блок предприятия и
    # блок на каждый аэропорт. В своде это строка предприятия и разбивка под
    # ней, а «Итого» складывается только по предприятиям — иначе аэропорты ФКП
    # попали бы в итог дважды, потому что сводный блок и есть их сумма.
    #
    # Ссылка на ту же таблицу, а не отдельный справочник предприятий:
    # предприятие само сдаёт форму 15-ГА и имеет собственную отчётность, то есть
