...
def _ga15_summary_tags_by_code() -> Dict[str, str]:
    """Код показателя строки 08 бланка → метка его графы."""
    return {
        code: tag
        for tag in GA15_METRIC_TAGS
        for code in _ga15_metric_code_candidates(GA15_SUMMARY_ROW_CODE, tag)
    }
...
def _ga15_summary_row(
    ...
    for block in blocks:
        for tag in GA15_METRIC_TAGS:
            ...
            if not _ga15_metric_in_filter(selected, GA15_SUMMARY_ROW_CODE, tag):
                row[key] = GA15_FILTERED_OUT
                continue
            total = Decimal("0")
            for month in block.months:
                total += buckets.get(month, {}).get(tag, Decimal("0"))
            row[key] = _dec_to_float(total)
...
class DataController:
    ...
    def load_pivot_data(self, mode: int, filters: ReportFilters, entity_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        if mode == MODE_AIRLINE:
            ...
            return self._load_pivot_ga15_summary(filters)
    ...
    def _load_pivot_ga15_summary(self, filters: ReportFilters) -> Dict[str, Any]:
        ...
        tag_by_code = _ga15_summary_tags_by_code()
        ...
        for row in rows:
            n_records += row.records
            code = (row.indicator_code or "").strip()
            tag = tag_by_code.get(GA15_CODE_ALIASES.get(code, code))
            if tag is None:
                continue
            year, month_name = _aggregate_period(row)
            if month_name not in MONTHS_LIST:
                continue
            month = (year, MONTHS_LIST.index(month_name) + 1)
            months_with_data.add(month)
            data[row.airport_id][month][tag] += _aggregate_total(row)
        ...
        with get_session() as session:
            ...
            selected = _ga15_selected_codes(session, f)
        ...
        if not shown:
            return self._load_pivot_ga15_summary_empty(
                headers, keys, groups,
                "В справочнике нет аэропортов. Загрузите отчёт или заведите их в «Справочниках».",
            )

        pivot_rows, n_airports, enterprises = self._ga15_summary_rows(
            shown, blocks, data, selected
        )
        ...
        return {
            ...
            "stats": {
                ...
                "indicators": sum(
                    ...
                    if _ga15_metric_in_filter(selected, GA15_SUMMARY_ROW_CODE, tag)
                ),
            },
        }

    @staticmethod
    def _ga15_summary_rows(
        shown: List[tuple],
        blocks: Sequence[Ga15PeriodBlock],
        data: Dict[int, Dict[tuple, Dict[str, Decimal]]],
        selected: Optional[Set[str]],
        ...
        for airport_id, name in top:
            pivot_rows.append(
                _ga15_summary_row(name, [airport_id], blocks, data, selected)
            )
    ...
    @staticmethod
    def _load_pivot_ga15_summary_empty(headers, keys, groups, message: str) -> Dict[str, Any]:
