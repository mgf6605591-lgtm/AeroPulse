...
Файл лежит рядом с базой, в каталоге данных пользователя: там же, куда программа
пишет всё остальное, и там же, откуда его попросят прислать при разборе проблемы.
"""
import logging
import sys
...
def log_path() -> Path:
    return get_data_dir() / LOG_FILE_NAME
...
def setup_logging(level: int = logging.INFO, path: Path | None = None) -> Path | None:
    ...
    Невозможность писать журнал не должна мешать работать: каталог профиля может
    оказаться недоступен — перемещаемый профиль без сети, кончившееся место, —
